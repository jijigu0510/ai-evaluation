...
// 로컬 환경을 위해 dotenv 로드 시도
try {
    const dotenv = require('dotenv');
    dotenv.config();
    console.log("✅ dotenv 모듈 로드 완료: .env 파일을 확인합니다.");
} catch (e) {
    console.log("ℹ️ [안내] dotenv 모듈이 설치되지 않았습니다. 로컬 환경이라면 터미널에서 'npm install dotenv'를 실행해주세요.");
}

// Render 등 환경 변수 설정에서 GEMINI_API_KEY 값을 가져옵니다.
...
app.post('/api/evaluate', async (req, res) => {
    ...
    try {
        ...
        if (!apiKey) {
            throw new Error('서버에 API 키가 없습니다. GEMINI_API_KEY 환경변수를 설정해주세요.');
        }
...
`;
        ...
        // 🔴 기존 코드에서 모델명을 gemini-1.5-flash-latest 로 변경하여 404 에러 방지
        ...
        if (!response.ok) {
            ...
            throw new Error(`구글 AI API 연동 실패: ${errorText}`);
        }
        ...
        // 클라이언트에 명확한 에러를 전달하기 위해 500 응답 코드를 사용합니다.
